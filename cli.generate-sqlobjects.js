...
exports.Run = function(params, options, onSuccess){
  ...
  .then(function(){ return new Promise(function(resolve, reject){
    if(params.OUTPUT_FILE){
      ...
      if(sqlobjectcount == 1){ for(var sqlobjectname in sqlobjects) txt = sqlobjects[sqlobjectname]; }
      else {
        txt += "{";
        var first = true;
        for(var sqlobjectname in sqlobjects){
  ...
  .then(function(){
    ...
    if(params.OUTPUT_FILE){
      ...
      for(var sqlobjectname in sqlobjects) console.log(sqlobjects_path + sqlobjectname + '.json');
    }
  })
