...
var xlib = {
  'spawn': function(path, params, cb, stdout, stderr, onError, options, onMessage){
    ...
    if(stderr) cmd.stderr.on('data',function(data){ data = data.toString(); stderr(data); if(didError) return; didError = true; if(onError) return onError(data); console.log(data); }); //stderr(data){ xxx; }
    cmd.on('error', function(err){ if(didError) return; didError = true; if(onError) return onError(err); console.log(err); });
