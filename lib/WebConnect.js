...
***************************/
var xlib = {
  'spawn': function(path, params, cb, stdout, stderr, onError, options){
    if(!options) options = {};
...
WebConnect.prototype.req = function (url, method, post, headers, ftarget, callback) {
  ...
  options.headers = {};
  if (global.authcookie) options.headers.Cookie = global.authcookie;
  if (post) {
  ...
  var req = browser.request(options, function (res) {
    // response is here
    if (global.debug) console.log(options.method + ' ' + url + "  Status: " + res.statusCode);
